...
for TableID in TablesList:
    ...
    AutoSkipperValue = 1 #AutoSkipper(TableID) #0
        ...
        try:
            ...
            if LoadedDependencies == True:
                ...
                if LoadType == 'Incremental Load':
                    ...
                    df = df.toDF(*[c.replace(' ', '_').replace(';', '_').replace('{', '_').replace('}', '_').replace('(', '_').replace(')', '_').replace('\n', '_').replace('\t', '_').replace('=', '_').replace('-', '_') for c in df.columns]) #account_number
                    df=df.dropDuplicates()
                    print("source count",df.count())
                    
                    #Performing the incremental load
                    df = DataTypeChange(df,DWHSchemaName,DWHTableName)

                    try:
                        IncrementalLoad(TableID, df)

                    except Exception as e:
                        logging.error(f"Error occurred while performing incremental load for table {TableID}: {e}")
                    ...
                    try:
                        ...
                        targetdataframe = get_data_quality_metrics(dataframe, sourcesystem, schemanames,table, previous_metrics_df,MergeKey,threshold_percentage=10)    
                    ...
                    try:
                        ...
                        logging.error(f"Error occurred while performing full load for table {TableID}: {e}")
                        ErrorMessage = str(e).split('java.lang.Exception: ')[0] if 'java.lang.Exception: ' in str(e) else str(e).split('at ')[0]
                    ...
                    try:
                        ...
                        logging.error(f"Error occurred while performing truncate and load for table {TableID}: {e}")
                        ErrorMessage = str(e).split('java.lang.Exception: ')[0] if 'java.lang.Exception: ' in str(e) else str(e).split('at ')[0]
                    try:
                        dataframe  = df
                        table = SrcTableName
