...
# MAGIC %md
# MAGIC **Notebook Name:** NB_Pivoted_GL_data <br>
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE VIEW finance.tab_GL_Data_Pivoted AS
# MAGIC
# MAGIC --start by pulling in axiom_gldaily2025 and pivoting all of the day columns (D1-D365)
# MAGIC
# MAGIC With daily2025 AS (
# MAGIC
# MAGIC SELECT ACCT, 
# MAGIC        DEPT, 
# MAGIC        DTYPE,
# MAGIC        DATE_ADD(DATE('2024-12-31'), CAST(SUBSTRING(Day, 2) AS INT)) AS Date,
# MAGIC           --number from the day column name is used to format dates (add to end of previous year)
# MAGIC        Value
...
# MAGIC )
# MAGIC WHERE LEFT(cast(ACCT as string), 1) IN ('1', '2', '3')
# MAGIC AND DTYPE = 'EOM'
# MAGIC --filter to balance sheet accounts (ACCT ID starting with 1, 2, or 3) and EOM values
# MAGIC ),
# MAGIC
# MAGIC --bring in December 2024 data so we can have comparisons to prior year end
# MAGIC --This is coming from axiom_gl2024 monthly table since we don't have daily 2024 data
# MAGIC
# MAGIC monthly2024 AS (
# MAGIC
# MAGIC SELECT ACCT, 
# MAGIC        DEPT, 
# MAGIC        DTYPE,
# MAGIC        DATE('2024-12-31') AS Date,
# MAGIC        M12 AS Value
# MAGIC FROM bronze.axiom_gl2024
# MAGIC WHERE LEFT(cast(ACCT as string), 1) IN ('1', '2', '3')
# MAGIC AND DTYPE = 'EOM'    
# MAGIC
# MAGIC ),
# MAGIC
# MAGIC --union 2025 data with Dec 2024 data
# MAGIC unioneddata AS (
# MAGIC
# MAGIC SELECT * FROM daily2025
# MAGIC UNION ALL
# MAGIC SELECT * FROM monthly2024
# MAGIC )
# MAGIC
# MAGIC --final output to select only necessary columns and join YRMODAY table
# MAGIC SELECT 
# MAGIC u.*,
# MAGIC y.Description,
# MAGIC TO_DATE(y.PriorDataDate, 'M/d/yyyy h:mm:ss a') AS PriorDate,
# MAGIC y.WkDay,
# MAGIC y.PriorWkDay,
# MAGIC y.YRQTR,
# MAGIC y.IsBD
# MAGIC FROM unioneddata AS u
# MAGIC LEFT JOIN bronze.YRMODAY AS y
# MAGIC ON u.date = MAKE_DATE(y.YearValue, y.MonthValue, y.DayValue);
