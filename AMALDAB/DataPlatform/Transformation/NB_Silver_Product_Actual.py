...
# base_df = spark.sql("select ProductCode from silver.product where currentrecord='Yes' group by 1")
...
# DBTITLE 1,Data from product_xref
try:
    logger.info("Joining base tabels for silver product table")
    ...
    basequery = '''
        ...
        ,ProductFamily
        ,ProductName
        from silver.product_xref
        where ProductFamily != 'Services'
        and CurrentRecord = 'Yes' '''
    df_final_FA = spark.sql(basequery)
    df_final_FA.createOrReplaceTempView("product_xref_VW")
...
# MAGIC %sql
# MAGIC Select count(1) from product_xref_VW
...
# DBTITLE 1,VW_Silver for consolidated view
...
# DBTITLE 1,mark the deleted rows
...
# DBTITLE 1,Dupes
# MAGIC %sql
# MAGIC Select ProductCode, count(1) from silver.product
...
# MAGIC %sql
# MAGIC Select count(1) from silver.product
# MAGIC where CurrentRecord='Yes'
