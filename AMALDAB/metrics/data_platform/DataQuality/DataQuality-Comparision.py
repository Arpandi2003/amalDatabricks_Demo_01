...
# MAGIC * **Created Date:** 09/04/2025
# MAGIC * **Created By:** Deeraj Rajeev
# MAGIC * **Modified Date:**
# MAGIC * **Modified By:** Deeraj Rajeev
# MAGIC * **Changes Made:**
...
# DBTITLE 1,backup to dqmetadata
# spark.sql("""
#     UPDATE config.dqmetadata
#     SET Load_type = (
#         SELECT LoadType 
#         FROM config.metadata
#         WHERE config.dqmetadata.table_name = config.metadata.DWHTableName
#         limit 1
#     )
#     WHERE EXISTS (
#         SELECT 1 
...
try:
    DF_MasterMetadata = spark.sql("""
        SELECT * FROM dqmetadata 
        WHERE ISActive != false
        and TABLE_ID IN ('B1', 'B4','B7','B8')
    """)
    ...
    for TableID in TablesList:
        try:
            ...
            if SchemaName not in "source":
                ...
                spark.sql("INSERT INTO DQlogs SELECT * FROM LogTable")
...
def comparing_metrics(source_df, dest_df):
    ...
    spark.sql("Insert into dqcomparison select * from result_with_comments")
...
try:
  ...
  for TableID in TablesList:  
    ...
    if SchemaName == 'metrics':
      SourceSchemaName=['identity_graph_api','minion_event', 'pulse_configuration']
      source_df = get_table(source_system,SourceSchemaName,RenamedTableName)
      dest_df = get_table(source_system,['metrics','pulse_configuration'], TableName)
...
def get_pipeline_summary(date):
    ...
    FROM 
        DQMetadata t
    LEFT JOIN 
        DQLOGS q ON t.Table_Name = q.Table_Name AND Cast(q.VALIDATION_DATE AS Date) =  Cast('{date}' AS Date)
    WHERE 
        t.ISACTIVE = TRUE
    GROUP BY 
...
def get_comparision_summary(date):
    summary_query = f""" SELECT  * from DQComparison WHERE VALIDATION_DATE = '{date}' """
    return spark.sql(summary_query)
